...
for i in range(0,len(d_t)):
    data = subprocess.Popen("./q2 {} {} {} {} {}".format(L, N_x[0], N_t*d_t[i], N_t, alpha),
...
for i in range(0,len(N_x)):
    data = subprocess.Popen("./q2 {} {} {} {} {}".format(L, N_x[i], T, N_t, alpha),
                            stdout=subprocess.PIPE).communicate()[0]
    a=data.split('\n')
    d_x = L/float(N_x[i])
    ...
    for k in range(0, N_x[i]+1):
        error2 += (heat_analytic[k]-heat_vector[k])**2
    RMSE_const_t.append(sqrt(error2/(N_x[i]+1)))

d_x = np.linspace(1/30., 1/10., num=10)
